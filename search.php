...
if (isset($_GET['hash'])) {
    ...
    if ($fileInfo) {
        ...
        if ($scanResults) {
            echo '<table border="1">';
            echo '<tr><th>Antivirus Name</th><th>Result</th><th>Scan Date</th></tr>';
            echo '<tr><td>Kaspersky</td><td>' . htmlspecialchars($scanResults['kaspersky_result']) . '</td><td>' . htmlspecialchars($scanResults['kaspersky_scan_date']) . '</td></tr>';
            echo '<tr><td>Trend Micro</td><td>' . htmlspecialchars($scanResults['trend_micro_result']) . '</td><td>' . htmlspecialchars($scanResults['trend_micro_scan_date']) . '</td></tr>';
            echo '<tr><td>ESET</td><td>' . htmlspecialchars($scanResults['eset_result']) . '</td><td>' . htmlspecialchars($scanResults['eset_scan_date']) . '</td></tr>';
            echo '</table>';
        } else {
